...
| `05-x87-fast` | x87 on the host FPU when the guest runs at 53/24-bit precision with round-to-nearest (Windows default / Direct3D): bit-exact vs. softfloat, falls back for everything else; `-cpu …,x87-fast=off` disables. Tests: `tools/x87-fast-test.c` (host oracle vs. real x87), `tools/x87-guest-test.py` (on/off identical under TCG). Super PI on the M1 Air alone: not measured (patch 06 supersedes it for the hot path) | upstream QEMU grows a floatx80 hardfloat path |
| `06-x87-inline-tcg` | **doc 13**: seven scalar binary64 TCG opcodes (x86-64 VEX+FMA3, aarch64 scalar FP) and a translator mode that keeps the x87 stack as host doubles across instructions at PC=53/RC=nearest/PE masked (TB flag), converting to x80 only at TB exits, before helpers and on faults (unwind repair via a third insn_start word). Same checks as x87-fast.h; anything else runs the helper sequence out of line and exits the TB, so bit-exact vs softfloat except empty registers after a pop. DOS loop 21.6 (softfloat) / 10.6 (patch 05) / 2.9 ns per op on x86-64; aarch64 bring-up done 2026-09-03 (doc 13 §Bring-up): UMOV element size and constant-into-V-register paths fixed; XP Super PI 1M on the M1 Air 9:49 → 1:57 (rig P4 1.7: 2:02), `x87-fast=off` control at softfloat pace, Win98 boots. Tests: `tools/x87-guest-test.py` (382k lines identical on/off) | upstream float ops in TCG, or a rewrite of the x87 translator upstream |
