...
| `40-d3dpt-device` | instantiate the paravirtual Direct3D device (`hw/d3dpt`, overlaid from `d3dpt/hw` by prepare; doc 14) on the pc machine next to the qemu-3dfx devices and add its meson subdir. The device itself is ours, not a patch: SysBus, register page at 0xdfffe000, 64 MiB RAM window at 0xd8000000, executor library dlopened on first guest attach | never (our device) |
