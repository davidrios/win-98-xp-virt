...
| `06-x87-inline-tcg` | **exploration (doc 13)**: five scalar binary64 TCG opcodes (x86-64 VEX+FMA3, aarch64 scalar FP) and an inline translator path for fld/fst m64, the six arithmetic ops (m64 and st(i) forms), fld st(i), fxch, fst st(i) at PC=53/RC=nearest/PE masked; same checks as x87-fast.h, branches to the helpers otherwise, so bit-exact. DOS loop 10.6 → 6.9 ns/op on x86-64; aarch64 lowering untested. Tests: `tools/x87-guest-test.py` (extended to every inlined form) | Level-3 design (doc 13) or upstream float ops in TCG |
