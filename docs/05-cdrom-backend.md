...
| VOB ProtectCD | launch check passes from a dump carrying both the data and the Q anomaly | **PASS** — The Settlers 3 in XP, both discs, from the `.ccd` **and** from the `.cue` (user, 2026-09-05): tutorial on CD1, campaign asks for and accepts CD2. The pair is an A/B, not a repeat: identical data anomaly (same 697 failing LBAs), Q replayed verbatim in one and synthesized in the other, so **the check reads the data anomaly and a dump without subchannel serves it** (doc 17 §2.6). The row's premise — that the dump must carry *both* — turns out to be wrong for this scheme |
