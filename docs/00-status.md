...
6. **M9** → `docs/tracks/m9-tcg-aarch64.md`: patches 17 (REP fast path)
   ...
   next the same-value skip's leftovers (the 6 % of changed-value patches
   still retranslate; the walk), a faster fps oracle for the games (the
   60-dumps/s probe saturates at ~40), then the user picks between the
   pinned guest registers and the HVF VM port the probe found feasible.
