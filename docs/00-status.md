...
4. **M5** → `docs/tracks/m5-cdrom-backend.md` (steps 1–7 done; step 8 as of 2026-09-05 has SafeDisc 2.x, plain mixed-mode + CD-DA and VOB ProtectCD all green — three of the six acceptance rows, SafeDisc 1.x retired as ill-posed; left: run the negative control (the clean discs are built; no check has ever been seen to fail), the FIFA 2002 no-match, SecuROM which needs DPM in `mds.rs`, multisession; M5e MDS done early, CHD open); M6 launcher.
