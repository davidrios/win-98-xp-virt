...
- d3dpt: `D3DPT_EXEC_LIB` / `D3DPT_DXVK_LIB` point the device and the executor
  ...
  QMP `system_powerdown` shuts XP down cleanly.
...
2. **M4 — paravirtual Direct3D device (doc 14, ADR-006, decided
   ...
   WineD3D-in-guest). **Next: P2** — resources (vertex/index buffers,
   textures with Lock/Unlock through `BUFFER_UPDATE`/`TEXTURE_UPDATE`,
   surfaces, GetBackBuffer/GetRenderTargetData, render targets) and
   shaders in the guest DLL until `D3DGAME9 -frames 600 -dump 300` runs
   on the device and diffs against the rig golden like the native build
   does; then the zero-copy present (DXVK Vulkan interop → dma-buf /
   IOSurface ring) instead of GetRenderTargetData; then the macOS build of
   `libd3dpt_exec` (KosmicKrisp). Fresh D3DPT DLL + ISO after every guest
