...
2. **M4 — paravirtual Direct3D device (doc 14, ADR-006, decided
   ...
   byte-identical to the native DXVK frame. **Next: P3** — a shader test
   with pre-assembled vs_1_1/ps_1_1 bytecode (this XP has no d3dx9),
   queries, state blocks, vertex declarations, cube/volume textures,
   DEFAULT-pool offscreen surfaces, then a real game (doc 04 matrix);
   the zero-copy present (DXVK Vulkan interop → dma-buf / IOSurface ring)
