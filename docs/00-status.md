...
| **M6 — launcher** (machine library, guided creation, snapshots UI, disc shelf, packaging) | `docs/tracks/m6-launcher.md` | `launcher/`, doc 07 | opened 2026-09-04 (worktree `.claude/worktrees/m6-launcher`, branch `track/m6-launcher`): UI toolkit decided (egui/eframe, not Slint — license fit); bundle format, library grid, spawning a player and the guided creation wizard (steps 1–4) landed, verified headlessly (no working GUI click automation on this session's Wayland setup — a human should click through the wizard once); next is snapshots UI + disc-shelf editing |
