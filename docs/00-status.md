...
| **M6 — launcher** (machine library, guided creation, snapshots UI, disc shelf, packaging) | `docs/tracks/m6-launcher.md` | `launcher/`, `player/src/shader.rs`, doc 07 | opened 2026-09-04 (worktree `.claude/worktrees/m6-launcher`, branch `track/m6-launcher`): UI toolkit decided (egui/eframe, not Slint — license fit); bundle format, library grid, spawning a player, the guided creation wizard with a native file picker, editing an existing machine in place (steps 1–4), and a shader profile manager (named presets + parameter overrides, assignable per machine) landed, verified headlessly + one real player dump-diff (no working GUI click automation on this session's Wayland setup — a human should click through the wizard and the shader manager once); next is snapshots UI + disc-shelf editing |
