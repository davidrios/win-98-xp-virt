...
| **M8 — CPU fast paths in TCG** (x87 shadows, SSE inline, TCG float opcodes) | `docs/tracks/m8-tcg-fastpaths.md` | `patches/qemu/05..07`, `tools/x87-*`, `tools/sse-guest-test.py`, `guest-tools/src/ssebench.c`, docs 13 and 16 | aarch64 validation of this session's x86-64-only work (MMX chain 2.1×, clamp+cmp `SSEBENCHC` 6.5×, both aarch64-unvalidated); an XP-guest `SSEBENCH.EXE` clamp+cmp number on this host to compare against the rig's 34 % |
