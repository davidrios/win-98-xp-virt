...
2. **M3 (doc 12), in progress:** ~~vtable patch → EGL backend with
   readback → Win98 wglgears in the player on Linux~~ (done) → macOS
   CGL/IOSurface backend (so the Air shows 3D in the player) → dma-buf /
