...
| **M7 — XP display driver** (`d3dpt-vga`, miniport + display DLL, DirectDraw/Direct3D DDI) | `docs/tracks/m7-display-driver.md` | `d3dpt/hw/d3dpt_vga.c`, `d3dpt/hw/d3dpt_exec_load.[ch]`, `d3dpt/d3dpt_fb.h`, `d3dpt/exec/d3dpt_exec_ddi.cpp`, `guest-tools/src/d3dptvid/`, `tools/xp-driver-test.sh`, `tools/xp-fifa2000.bat`, `tools/xp-fifa-match.sh`, `tools/xp-diablo.sh`, `tools/d3dpt-dp2-test.cpp`, doc 15 | **the DX8 DDI landed 2026-09-05** (`D3DCAPS8`, hardware T&L, the DX8 token stream rewritten by the driver, TEXBLT, state sets, render-to-texture; D3DGAME8 through XP's own d3d8.dll with hardware vertex processing); **open first:** Max Payne's clipped fans on it read from the wrong buffer (nearest walls / ground black; `ddflags=0x2000` = the DX7 face renders it clean), DXT textures never reach the driver; then shaders 1.x, more streams, colour keying; `ZBIAS` still dropped; the user confirms `D3DPT\DINPUT.DLL` fixes the TCG match keyboard; then colour keying / T&L / the DX8 DDI as the next title demands, more 8 bpp titles (StarCraft, AoE) |
...
M7c HAL, the DirectX 8 DDI with hardware T&L (D3DGAME8 and Max Payne through XP's own d3d8.dll), Diablo on the new 8 bpp modes; the user confirms the `DINPUT.DLL`
keyboard fix under TCG, then colour keying / T&L / DX8 tokens, more 8 bpp
titles, a driver stage in `scripts/test.sh`, cursor / vblank / mode table). ADR-008 (2026-09-04): the M7 driver is the long-term XP shape; the
