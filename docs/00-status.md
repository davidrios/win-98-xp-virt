...
6. **M9** → `docs/tracks/m9-tcg-aarch64.md`: patches 17 (REP fast path)
   18 (same-value SMC stores: Moto Racer's race 3×) and 19 (the store
   slow path's nested RCU locks, TLS on macOS) done 2026-09-05;
   next the same-value skip's leftovers (the 6 % of changed-value patches
   still retranslate; the walk), then the user picks between the
   translator work (inline TB lookup, pinned guest registers) and the HVF
   VM port the probe found feasible.
