...
| **M5 — CD-ROM backend** (libdisc, `cdimage` block driver, ATAPI/MMC, CD-DA) | `docs/tracks/m5-cdrom-backend.md` | `libdisc/`, `patches/qemu/50..59`, `tools/atapi-guest-test.py`, `guest-tools/src/cdtest.c`, docs 05 and 17 | steps 1–4 landed 2026-09-04 (model, cue/bin + CCD + ISO, EDC/ECC, Q, MMC responders, C API, `discx`, the `cdimage` block driver + patch 50, XP copies a converted disc); next step 5: patch 51 (ATAPI data path, TOC, subchannel, `tools/atapi-guest-test.py`) |
...
4. **M5** → `docs/tracks/m5-cdrom-backend.md` (steps 1–4 done 2026-09-04; step 5: patch 51, the ATAPI data path + TOC + subchannel from the model, spec doc 17 §5.3); M6 launcher.
