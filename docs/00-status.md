...
2. **M4 — paravirtual Direct3D device (doc 14, ADR-006, decided
   ...
   (0.35 % of pixels beyond tolerance 8, see the state table); the Air
   only has to confirm KosmicKrisp reproduces it. Then P1 (transport +
   device), which runs entirely on Linux: XP image copied from the Air.
