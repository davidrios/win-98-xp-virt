...
| **M8 — CPU fast paths in TCG** (x87 shadows, SSE inline, TCG float opcodes) | `docs/tracks/m8-tcg-fastpaths.md` | `patches/qemu/05..07`, `tools/x87-*`, `tools/sse-guest-test.py`, `guest-tools/src/ssebench.c`, docs 13 and 16 | **ready to merge to `main`** (2026-09-04: aarch64-validated on the Air, both batteries identical; XP `SSEBENCH.EXE` on the x86-64 box clamp+cmp 43 % of the rig vs the Air's 34 %, the loop's remaining cost being its load/store TLB lookups and the stock `movmskps` helper); after the merge, item 2 of the track doc — a real Direct3D workload with and without `*-fast=off` |
