...
ADR-008 (2026-09-04): a real XP display driver is the long-term shape (M7, staged: framebuffer → DirectDraw DDI → Direct3D DDI) on the same transport and executor; the DLL device stays for Win98 and as the harness. **M7a landed the same day (doc 15, branch worktree-luminous-dancing-cocke, in parallel with M4's next steps).** Next on M7: (a) hardware cursor through the device so the pointer leaves the dirty-rect stream, (b) the mode table from the player (M2, pixel aspect), (c) a per-frame vblank signal, (d) macOS run of the same image, then **M7b**: DirectDraw DDI (`DrvEnableDirectDraw`, surfaces in VRAM, blits as records over a doorbell on BAR 1; needs the DirectDraw DDI header written from the public docs, mingw has none), exit = FIFA 2000 without WineD3D.
