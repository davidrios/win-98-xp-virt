...
# XP in the player: replace -hda/-vga with (doc 15; the driver is installed
# from the ISO's DRIVER\ folder once, DRVINST.EXE -reboot). On a Linux
# host add -accel kvm -cpu host (instead of -cpu pentium3): much faster
# than TCG, which stays the Apple Silicon path.
#   -hda ~/vms/winxp.qcow2 -vga none -device d3dpt-vga
...
- Kernel-mode drivers with mingw-w64 (doc 15): miniport = `ntdef.h` +
  `ddk/miniport.h` + `ntddvdeo.h` + `ddk/video.h`, never `ntddk.h` (enum
  conflicts; and mingw's `wdm.h` does not compile with
  `_WIN32_WINNT=0x0501`); `winddi.h` needs `ddrawint.h`/`d3dnthal.h` that
  mingw 14 lacks (`guest-tools/src/d3dptvid/ddk-stubs/`); GCC calls
  `memcpy`/`memset` for struct copies → `kcrt.c`; the .sys must import only
  videoprt.sys, the display DLL only win32k.sys (the build script checks).
  Debug output = one character per write to the device's DEBUG register,
  shown as `d3dpt-vga: guest: …` in the QEMU log. `grim` hangs inside the
  agent sandbox; QMP `screendump` works for the framebuffer driver (the
  console surface *is* VRAM) and the player log shows the switches.
  Keys typed while a full-screen DirectDraw window is up go to that window
  and are lost: chain the guest commands with `&` on one `cmd /k` line
  (`qmpc.py type` knows `& ( ) , ; = ' " * % + !` now) and copy logs to a
  FAT scratch disk (`-hdb`, `mcopy -i img@@1048576`) at the end.
  `-device d3dpt-vga,ddflags=N` flips display-driver behaviours from the
  host for bisection without a reinstall (doc 15).
- Driving XP from a script (2026-09-04): `qmpc.py … keys esc` closes the
  balloon/wizard, `keys meta_l+r`, `type 'cmd /k D:\DRIVER\SETMODE.EXE
  1024 768 32 85'`, `keys ret` keeps the console open with the output;
  `type 'shutdown -r -t 0'` restarts; the CD swaps at runtime with QMP
  `blockdev-change-medium` (device `ide1-cd0`). XP's Logo-test dialog:
  `keys alt+c` = Continue anyway.

## Next steps, in order

ADR-008 (2026-09-04): a real XP display driver is the long-term shape (M7, staged: framebuffer → DirectDraw DDI → Direct3D DDI) on the same transport and executor; the DLL device stays for Win98 and as the harness. **M7a landed the same day (doc 15, branch worktree-luminous-dancing-cocke, in parallel with M4's next steps).** Next on M7: (a) hardware cursor through the device so the pointer leaves the dirty-rect stream, (b) the mode table from the player (M2, pixel aspect), (c) a per-frame vblank signal, (d) macOS run of the same image, ~~then **M7b**: DirectDraw DDI~~ first cut landed 2026-09-04 (VRAM surfaces, page flips, cached mappings, DDTEST numbers in doc 15). Next: a 2D DirectDraw title on it, then **M7c** (Direct3D DDI: `D3dContextCreate`, `D3dDrawPrimitives2` DP2 tokens → executor records through a doorbell on BAR 1, textures as VRAM surfaces uploaded to the host), exit = FIFA 2000 / the doc 04 matrix without WineD3D.
