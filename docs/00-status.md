...
| Guest tools | `guest-tools/build-wrappers.sh` builds the qemu-3dfx guest wrappers (msvcrt-linked, `-march=pentium3`, wglgears test EXE) and, since 2026-09-03, the WineD3D set from JHRobotics/wine9x (Wine 1.7.55 for 9x/XP: per-game D3D8/D3D9/WINED3D DLLs + system-wide switchers) with a D3D9 smoke test (`D3D9TEST.EXE`) into an ISO. Must match the host's qemu-3dfx commit. **Win98 (2026-09-03): wglgears and D3D9TEST both run in the player** (WineD3D needs no Microsoft DX runtime in the guest). XP: not yet, see open threads. |
...
- XP: wglgears and D3D9TEST "crash at startup" (2026-09-03). On NT the
  qemu-3dfx OPENGL32.DLL maps the device through `\\.\MAPMEM` = FXPTL.SYS
  in its DllMain and returns FALSE without it, so every GL/D3D EXE fails
  to initialize; the ISO's WIN2KXP step (FXPTL.SYS + INSTDRV.EXE as admin)
  is required for OpenGL, not only Glide. Confirm that first; if the
  driver is installed (`sc query fxptl`), look for `mesapt:` lines on the
  player's stderr and the guest's Dr. Watson log.
...
1. ~~M1~~ closed 2026-09-02 (latency at the vsync floor, XP benchmarked,
   ...
   Next on x87: in XP, `D3D9TEST.EXE` from the guest-tools ISO next to
   D3D9.DLL + WINED3D.DLL + OPENGL32.DLL (expect PC=24 in its output and
   a WineD3D adapter string), then a Direct3D title with and without
