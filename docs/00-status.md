...
| D3D executor (M4) | **ADR-007 (2026-09-03): DXVK d3d9 native everywhere; macOS over Mesa's KosmicKrisp (needs macOS 26), MoltenVK unsupported** — spike C (`docs/spikes/spike-c-dxvk-native-macos.md`): DXVK master hard-requires five features MoltenVK 1.4.2 lacks. `third_party/dxvk` submodule (3.1.0 d7ac258), patch queue `patches/dxvk/`. DXVK core compiles on macOS with Apple clang; Windows-shim portability patch in the queue. |
